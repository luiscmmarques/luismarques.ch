...
| `src/pages/index.astro` | The single page: layout, inline script, page-scoped styles, meta/SEO tags, CSP. Also `buildVersion()`, which stamps the footer with the build date plus short commit (same identifier Graftful and coruja print), resolved at build time from git and falling back to `unknown` when there is no git. |
...
| `public/` | `logo.svg` (hand-drawn, and the only icon that is: it is the SVG favicon and the source every raster is rendered from), the app icon set (`favicon.ico`, `apple-touch-icon.png`, `icon-192.png`, `icon-512.png` and the two `-maskable` variants, all generated, all opaque tiles), `manifest.webmanifest`, `og.png` (the link preview card, generated from `tools/og-card.html`, not hand-edited), `luis.jpg` (portrait used by the JSON-LD `image`, kept as jpg for scrapers), `luis-sm.webp` + `luis-sm.jpg` (page avatar, WebP first with a jpg fallback; regenerate with `cwebp -q 82 public/luis-sm.jpg -o public/luis-sm.webp`), `CNAME`, `robots.txt`, `sitemap.xml`, `llms.txt`, `humans.txt`, `.well-known/security.txt` (RFC 9116, has an Expires date to renew yearly). |
| `tools/` | Build-time helpers, excluded from the site. `og-card.html` is the 1200x630 source of `public/og.png`; `og-optimize.py` palettes the screenshot down from around 210 KB to around 70 KB. `icon-card.html` is the 512x512 source of the app icon tile; `icons.py` derives every other size, both maskable variants and `favicon.ico` from that one render. Regeneration commands are in each card's header comment. Nothing here ends up in `dist/`. |
...
- **App icons**: after changing `logo.svg` or the theme, regenerate the set (see `tools/icon-card.html`). The rule the set encodes: the SVG favicon stays the bare outline, because a tab strip picks its own background colour and an outline reads on a light one and a dark one alike, while every raster is a filled tile, because iOS composites `apple-touch-icon` on black and a launcher draws a manifest icon on a wallpaper. `icons.py` refuses a transparent render for exactly that reason. The maskable pair is inset to 57% of the canvas, which is the largest square inside Android's guaranteed centre circle.
